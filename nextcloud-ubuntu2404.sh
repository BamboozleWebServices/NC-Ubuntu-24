#!/bin/bash
##########################################################################################
# Ubuntu 24.04 LTS x86_64  (interactive build)
# Nextcloud (latest stable)
# Interactive Nextcloud installer for Ubuntu 24.04 LTS
##########################################################################################

INSTALLATIONFILE="nextcloud-ubuntu2404.sh"
# URL to YOUR hosted copy of the Nextcloud update script (see README)
UPDATESCRIPTURL="https://raw.githubusercontent.com/BamboozleWebServices/NC-Ubuntu-24/main/update.sh"
#
#########################################################################
...
then
    ...
    echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    echo ""
    echo -e " » \e[1;31mNO ROOT PERMISSIONS!\033[0m"
    echo ""
    echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    echo " » Please run the script as root: 'sudo ./$INSTALLATIONFILE'"
    echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
    echo ""
    exit 1
fi
...
##################################################
# Interaktive Konfiguration / Interactive setup  #
# (replaces the old zero.cfg file ingest)         #
##################################################
clear
echo ""
echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
...
echo " NOTE: Let's Encrypt is enabled by default, so this MUST be a public"
echo "       domain that resolves to this server with ports 80 + 443 open."
echo "       For a LAN-only install, set LETSENCRYPT=\"n\" in the fixed block below."
echo ""
while [ -z "$NEXTCLOUDDNS" ]; do
  read -rp " » Public domain for Nextcloud (e.g. cloud.example.ae): " NEXTCLOUDDNS
done

# --- Admin username ---
read -rp " » Nextcloud admin username [ncadmin]: " NEXTCLOUDADMINUSER
NEXTCLOUDADMINUSER="${NEXTCLOUDADMINUSER:-ncadmin}"

# --- Admin password ---
echo ""
echo " » Admin password:"
echo "     1) Auto-generate  (recommended - shown at the end of install)"
echo "     2) Type my own"
read -rp " » Choice [1]: " _pwchoice
if [ "${_pwchoice:-1}" = "2" ]; then
  while [ -z "$NEXTCLOUDADMINUSERPASSWORD" ]; do
    read -rsp " » Enter admin password: " NEXTCLOUDADMINUSERPASSWORD; echo ""
  done
else
  NEXTCLOUDADMINUSERPASSWORD=$(openssl rand -hex 16)
fi

# --- Region: timezone + phone region (multiple choice) ---
echo ""
echo " » Region (sets timezone + Nextcloud phone region):"
echo "     1) UAE      (Asia/Dubai, AE)"
echo "     2) Germany  (Europe/Berlin, DE)"
echo "     3) Austria  (Europe/Vienna, AT)"
echo "     4) UK       (Europe/London, GB)"
echo "     5) Custom"
read -rp " » Choice [1]: " _region
case "${_region:-1}" in
  2) CURRENTTIMEZONE='Europe/Berlin'; PHONEREGION='DE' ;;
  3) CURRENTTIMEZONE='Europe/Vienna'; PHONEREGION='AT' ;;
  4) CURRENTTIMEZONE='Europe/London'; PHONEREGION='GB' ;;
  5) read -rp "   - Timezone (e.g. Asia/Dubai): " CURRENTTIMEZONE
     read -rp "   - Phone region (e.g. AE): " PHONEREGION ;;
  *) CURRENTTIMEZONE='Asia/Dubai'; PHONEREGION='AE' ;;
esac

# --- Max upload size ---
echo ""
read -rp " » Max upload size [10G]: " UPLOADSIZE
UPLOADSIZE="${UPLOADSIZE:-10G}"

#################################################################
# Fixed defaults - not prompted on purpose.                     #
# Edit any of these here to change the baked-in behaviour.      #
#################################################################
PHPVERSION="8.4"            # fixed
...
# lowercase the DB identifiers (same as the original script did)
NCDBNAME="${NCDBNAME,,}"
NCDBUSER="${NCDBUSER,,}"
...
echo " +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "   Domain ............: $NEXTCLOUDDNS"
echo "   Admin user ........: $NEXTCLOUDADMINUSER"
...
case "${_confirm,,}" in
  y|yes) : ;;
  *) echo ""; echo " » Aborted. Nothing was changed."; exit 1 ;;
esac
sleep 1
...
echo -e " »\e[1;31m"
echo "****************************************"
echo "* You aren't operating on Ubuntu 24.04 *"
echo "****************************************"
echo  -e "\033[0m"
echo ""
exit 1
...
# Uninstall script        #
###########################
mkdir -p /home/"$LINUXUSER"/nextcloud-installer/
touch /home/"$LINUXUSER"/nextcloud-installer/uninstall.sh
cat <<EOF >/home/"$LINUXUSER"/nextcloud-installer/uninstall.sh
#!/bin/bash
if [ "\$(id -u)" != "0" ]
then
clear
...
rm -Rf /etc/ufw /etc/crowdsec /var/www /etc/mysql /etc/postgresql /etc/postgresql-common /var/lib/mysql /var/lib/postgresql /etc/letsencrypt /var/log/nextcloud /home/"$LINUXUSER"/"$NEXTCLOUDDNS"/nextcloud-installer/install.log /home/"$LINUXUSER"/"$NEXTCLOUDDNS"/nextcloud-installer/update.sh
...
exit 0
EOF
chmod +x /home/"$LINUXUSER"/nextcloud-installer/uninstall.sh
##########################
...
if [ -e "/var/www/nextcloud/config/config.php" ] || [ -e /etc/nginx/conf.d/nextcloud.conf ]; then
  ...
  echo "* /home/$LINUXUSER/nextcloud-installer/uninstall.sh"
  echo ""
  exit 1
else
  echo "*************************************************"
...
if [ ! -d "/home/$LINUXUSER/nextcloud-installer/" ]; then
  echo -e "* Creating: Install directory .......:::::::\033[32m OK\033[0m *"
  mkdir /home/"$LINUXUSER"/nextcloud-installer/
  echo ""
  else
  ...
  seconds=10
  while [ $seconds -gt 0 ]; do
   echo -ne "* INSTALLATION STARTS IN $seconds\033[0K\r"
   sleep 1
   : $((seconds--))
  done
  echo ""
...
# install.log             #
###########################
exec > >(tee -i "/home/$LINUXUSER/nextcloud-installer/install.log")
exec 2>&1
###########################
# E: Update-function      #
###########################
...
http {
  log_format nc_json escape=json
  '{'
  ...
  access_log /var/log/nginx/access.log nc_json;
...
${wget} -q https://download.nextcloud.com/server/releases/$NCRELEASE & spinner
...
#${clear}
#${echo} ""
#${echo} "CHECKSUM ERROR => SECURITY ALERT => ABORT!"
#exit 1
#fi
${echo} "Extracting:" $NCRELEASE
${apt} install -y bzip2
${tar} -xjf $NCRELEASE -C /var/www & spinner
...
${echo} ""
${sudo} -u www-data /usr/bin/php -f /var/www/nextcloud/cron.php & spinner
###########################
# Hold software           #
###########################
setHOLD
...
###########################
cd /home/"$LINUXUSER"/
${wget} -q -O update.sh "$UPDATESCRIPTURL"
${chmod} +x /home/"$LINUXUSER"/update.sh
###########################
# E: Final screen         #
###########################
${clear}
